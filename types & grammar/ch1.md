...
> Q. `[[Call]]` property는 어떠한 속성인가? 변수에 값을 넣고 값을 호출하는 것과는 어떤 차이가 있을까?
>
> A.
...
> if문 안에서 atob을 var로 선언하면(심지어 if문 조건을 통과를 안해도) atob이 전역(the top of the scope?)으로 호이스팅 된다라.  
typeof atob이 "ReferenceError: atob is not defined"가 아니라 "undefined" 즉 변수는 선언하고 값을 할당하지 않은 상태 일 때만 if문 타면서 값을 할당한다? 오호라
...
> Q. 이 경우에 에러가 안난다고 해도 FeatureXYZ를 실행 할 때 내가 원하는 코드가 실행 되는걸 보장해 주지는 않잖아?
>
> A.
